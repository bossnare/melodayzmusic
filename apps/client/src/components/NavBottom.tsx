    ...
    <nav className="sticky bottom-0 left-0 right-0 flex items-center justify-center w-full gap-6 px-2 text-sm border-t border-t-gray-200 md:text-base sm:px-0 h-18 md:h-14 bg-gray-50 md:gap-5 md:py-5 lg:gap-2">
      ...
      {nav.map((tab) =>
        tab.label === 'button' ? (
          ...
          <Link
            className="flex flex-col items-center justify-center gap-1 py-1 text-xs font-semibold md:gap-4 md:flex-row bg-amber-20 md:text-base w-22 md:w-40 min-h-14 max-h-14"
            href={'tab.route'}
            key={tab.id}
          >
            <>
