...
const AuthPageWrapper = ({
  ...
  return (
    <section className="relative flex flex-col items-center justify-start gap-6 pb-6 sm:justify-between lg:gap-2 sm:h-auto h-dvh lg:h-dvh md:pb-4">
      {children}
      {/* Overlay */}
      <Overlay isPending={isPending} textLoading={textLoading} />
      {/* tag */}
      <footer className="absolute bottom-0 left-0 flex flex-col items-center sm:static">
        <Tagline />
      </footer>
    </section>
  );
